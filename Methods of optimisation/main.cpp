...
int main() {
    ...
    /*
    {
        ...
        exp3_results << "N=" << N << ", T=" << T << ", dt=" << dt << "\n";
        exp3_results << "Test matrix: H_test_matrix.txt\n";
        exp3_results << "Reference: Runge-Kutta 4th order\n";
        exp3_results << "-----------------------------------------------------------------\n";
        exp3_results << "Method | Steps | Max Element Diff | Max Eig Diff | Time (ms)\n";
        exp3_results << "-----------------------------------------------------------------\n";

        // Reference solution using Runge-Kutta
        ...
        for (int step = 0; step < num_steps; ++step) {
            U_rk = runge_kutta_simple::runge_kutta_step(H_test, U_rk, dt, N);
        }
        ...
        std::cout << "Reference (Runge-Kutta) computed in " << time_rk_ms << " ms\n";

        // Compare with exact solution U(T) = exp(-i * H_test * T)
        // This is the theoretical solution for constant H
        CMatrix Omega_exact = matrix_ops::mat_scale(H_test, complexd(0.0, -T));
        CMatrix U_exact = matrix_ops::expm_pade_eigen(Omega_exact, N);

        // Compare Runge-Kutta with exact solution
        ...
        exp3_results << "Runge-Kutta vs Exact | " << num_steps << " | " << max_el_diff_rk << " | " << max_eig_diff_rk << " | " << time_rk_ms << "\n";

        // Also compare different time steps for Runge-Kutta
        std::vector<double> dt_values = {dt, dt*2, dt*5, dt*10};
        for (double current_dt : dt_values) {
            ...
            CMatrix U_test = utils::eye(N);
            for (int step = 0; step < steps; ++step) {
                U_test = runge_kutta_simple::runge_kutta_step(H_test, U_test, current_dt, N);
            }
            ...
            CMatrix diff(N * N, complexd(0.0, 0.0));
            matrix_ops::mat_sub(U_test, U_exact, diff, N);

            double max_el_diff = matrix_ops::max_element_diff(U_test, U_exact, N);
            double max_eig_diff = matrix_ops::max_eigenvalue_modulus_hermitian(diff, N);

            exp3_results << "RK dt=" << current_dt << " | " << steps << " | " << max_el_diff << " | " << max_eig_diff << " | " << time_ms << "\n";
        }

        exp3_results << "-----------------------------------------------------------------\n";
        exp3_results << "Note: Exact solution uses exp(-i*H*T) with Eigen Pade approximation\n";
        exp3_results.close();
