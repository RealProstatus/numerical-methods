...
// Изменяем реализацию
    ...
    CMatrix expm_pade(const CMatrix& A, int N, int p, int q) {
        // Пока поддерживаем только [3/3] Pade для простоты
        if (p != 3 || q != 3) {
            std::cerr << "Only [3/3] Pade is implemented." << std::endl;
            return utils::eye(N); // Возврат I на ошибке
        }

        // 1. Оценка нормы и scaling
        double norm = mat_one_norm(A, N);
        if (norm == 0.0) {
            return utils::eye(N); // exp(0) = I
        }

        // Threshold для [3/3] Pade: норма < 1 для сходимости
        const double theta = 1.0; // Для [3/3]
        int s = static_cast<int>(std::ceil(std::log2(norm / theta)));
        if (s < 0) s = 0; // Минимальный scaling

        // A_scaled = A / 2^s
        complexd scale_factor(1.0 / std::pow(2.0, s), 0.0);
        CMatrix A_scaled = mat_scale(A, scale_factor);

        // 2. Вычисление powers of A_scaled для Pade
        CMatrix A2(N * N, complexd(0.0, 0.0)); // A^2
        matmul(A_scaled, A_scaled, A2, N);

        CMatrix A3(N * N, complexd(0.0, 0.0)); // A^3
        matmul(A2, A_scaled, A3, N);

        // [3/3] Pade: p(A) = 120 I + 60 A + 12 A^2 + A^3
        // q(A) = 120 I - 60 A + 12 A^2 - A^3 (нормализованные коэффициенты)
        CMatrix p_mat = utils::eye(N);
        mat_scale_inplace(p_mat, N, complexd(120.0, 0.0));

        CMatrix temp(N * N, complexd(0.0, 0.0));
        for (int i = 0; i < N * N; ++i) temp[i] = A_scaled[i];
        mat_scale_inplace(temp, N, complexd(60.0, 0.0));
        mat_add(p_mat, temp, p_mat, N);

        for (int i = 0; i < N * N; ++i) temp[i] = A2[i];
        mat_scale_inplace(temp, N, complexd(12.0, 0.0));
        mat_add(p_mat, temp, p_mat, N);

        mat_add(p_mat, A3, p_mat, N);

        // q(A) = 120 I - 60 A + 12 A^2 - A^3
        CMatrix q_mat = utils::eye(N);
        mat_scale_inplace(q_mat, N, complexd(120.0, 0.0));

        for (int i = 0; i < N * N; ++i) temp[i] = A_scaled[i];
        mat_scale_inplace(temp, N, complexd(-60.0, 0.0));
        mat_add(q_mat, temp, q_mat, N);

        for (int i = 0; i < N * N; ++i) temp[i] = A2[i];
        mat_scale_inplace(temp, N, complexd(12.0, 0.0));
        mat_add(q_mat, temp, q_mat, N);

        for (int i = 0; i < N * N; ++i) temp[i] = A3[i];
        mat_scale_inplace(temp, N, complexd(-1.0, 0.0));
        mat_add(q_mat, temp, q_mat, N);

        // 3. exp_approx = p / q = p * inv(q)
        // Инверсия q_mat: Используем LU decomposition (LAPACKE_zgetrf + zgetri)
        CMatrix q_copy = mat_copy(q_mat); // Копия, т.к. zgetrf разрушает матрицу
        std::vector<lapack_int> ipiv(N); // Pivot indices

        // LU factorization
        lapack_int info = LAPACKE_zgetrf(LAPACK_ROW_MAJOR, N, N,
            reinterpret_cast<lapack_complex_double*>(q_copy.data()), N, ipiv.data());
        if (info != 0) {
            std::cerr << "LAPACKE_zgetrf failed with info = " << info << std::endl;
            return utils::eye(N);
        }
        ...
        info = LAPACKE_zgetri(LAPACK_ROW_MAJOR, N,
            reinterpret_cast<lapack_complex_double*>(q_copy.data()), N, ipiv.data());
        if (info != 0) {
            std::cerr << "LAPACKE_zgetri failed with info = " << info << std::endl;
            return utils::eye(N);
        }

        // exp_approx = p * inv(q)
        CMatrix exp_approx(N * N, complexd(0.0, 0.0));
        matmul(p_mat, q_copy, exp_approx, N);

        // 4. Squaring: (exp_approx)^{2^s}
        CMatrix result = exp_approx;
        CMatrix sq_temp(N * N, complexd(0.0, 0.0));
        for (int i = 0; i < s; ++i) {
            matmul(result, result, sq_temp, N);
            result = sq_temp;
        }
